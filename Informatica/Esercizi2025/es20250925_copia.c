        ...
        switch (scelta)
        {
            ...
            case 5:
            {
                ...
                for(i = 0; i < n; i++)
                {
                    if(array[i] % 2 != 0)
                    {
                        int *tmp = realloc(array_dispari, (cont_dispari + 1) * sizeof(int));    // +1 server per allocare lo spazio per il nuovo elemento // cont_dispari lo alloca a 0
                        if(tmp == NULL)
                        {
                            printf("Errore di allocazione della memoria con realloc\n");
                            free(array_dispari);
                            break;
                        }
                        array_dispari = tmp;
                        array_dispari[cont_dispari] = array[i];
                        cont_dispari++;
                    }
                }
                ...
                if(cont_dispari == 0)
                {
                    printf("Nessun valore dispari trovato.\n");
                }
                else
                {
